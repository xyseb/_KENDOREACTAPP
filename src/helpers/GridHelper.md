# GridHelper original fournis par kendo-react
> Fichier versionné au format markdown afin de ne pas être compilé et d'avoir un visuel sur le code de GridHelper.tsx
fourni par la documentation de kendo-react (https://www.telerik.com/kendo-react-ui/components/grid/getting-started/gridhelper).
Il permettra de voir les changements qu'apporte Telerik à ce fichier au fil du temps car celui-ci n'est pas versionné chez eux.
## Contenu
```ts
                ...
                if (props.dataItemKey || GridProps.dataItemKey) {
                    let selectedItems = data.filter((item) => {
                        return newSelectedState[
                            item[props.dataItemKey ?? GridProps.dataItemKey]
                        ];
                    });
                    props.onSelectedItemsChange({ selectedItems: selectedItems });
                } else {
                    console.warn(
                        "The 'dataItemKey' property must be set for the GridHelper or the Grid in order to use 'onSelectedItemsChange'"
                    );
                }
            }
        },
