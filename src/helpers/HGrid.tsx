...
type HSortable = {
    /** Mode tri sur une colonne unique ou sur de multiple colonnes */
    mode: 'single' | 'multiple';
    /** Permet de ne pas pouvoir annuler le tri (si undefined, par défaut vaut true) */
    allowUnsort?: boolean;
    /** fonction de tri */
    //onSort: (event: GridSortChangeEvent) => void;
    /** Trie apliqué par défaut à la grille */
...
interface IHGridOwnProps<T, K extends keyof T> extends Omit<GridProps,
    ...
    "sortable" | "defaultSort"| "sort" | "onSortChange" | // usage similaire mais simplifié. sort et onSortChange devienne inutile
    "skip" | "take" | "pageSize" | "onPageChange" | "filter" | "onFilterChange" | "group" | "onGroupChange" | // props qui devienne inutile en mode controllé
    "navigatable" |
    "autoProcessData" | // On utilise pas le mode buit-in mais le mode controllé donc cette props ne sera pas disponible
    "style" | // On passera par un className et une definition dans le scss
    "loader" | // On va donner toujours le même loader
    "locale" | "language" // uniquement SSR (server side rendering) donc cette props ne sera pas disponible
    >
{
    /**
     ...
     */
    hToolbarSettingsProp?: IToolbarSettings;
    /**
    ...
    hSelectableProp?: HSelectable;

    hSortableProp?: HSortable;
...
//export const GridHelper = <T,>(props: Readonly<IGridHelperOwnProps<T>>): JSX.Element =>
{
    ...
    // Destructuration de toutes les props pour clarifier l'usage de façon claire
    const {hToolbarSettingsProp, hSelectableProp, hSortableProp, hColumnsProps, ...gridProps} = props;

    // Est groupable par defaut si droit de trier grouper filter
    const isGroupable = gridProps.groupable !== undefined
    ...
    const isSortable = gridProps.sortable !== undefined
        ? DroitTrierGrouperFilterRechercherHGrid(utilisateur)
        : false;
    console.log("isSortable",isSortable);
    // Est filtrable par defaut si droit de trier grouper filter
    const isFilterable = gridProps.filterable !== undefined
        ? DroitTrierGrouperFilterRechercherHGrid(utilisateur)
        : false;
    console.log("isFilterable",isFilterable);

    const pageable = gridProps.pageable;
    ...
    const sortable: SortSettings | undefined = gridProps.sortable && { allowUnsort: true, mode: 'multiple' };

    const toolbarSettings = useMemo<IToolbarSettings>(() => hToolbarSettingsProp ?? {
    ...
    const [data, setData] = useState(unfilteredData.map(di => di.dataItem));
        ...
        if (onSelectCallback !== undefined)
        {
            ...
            const newSelection = (event.dataItems as DataGridItem<T>[]) // Étape 1 : réduction pour obtenir les dataItems purs
                ...
                .reduce((acc, currentDataItem) => // Étape 2 : construction de la nouvelle sélection complète
                {
                    ...
                    {
                        if (hSelectableProp?.selectMode === HSelectMode.Cell) // === mode single cell ===
                        {
                            const colIndex = (event.select[String(currentDataItem[gridProps.dataItemKey])] as number[] | undefined)?.[0];
                        ...
                        {
                            const currentKey = String(currentDataItem[gridProps.dataItemKey]);
                            ...
                            const existing = (previousSelection as { dataItem: T; colFields: string[] }[])
                                .find(sel => sel.dataItem[gridProps.dataItemKey] === currentDataItem[gridProps.dataItemKey]);
                            ...
                            if (selectedColFields.length === 0)
                            {
                                ...
                                return (acc as { dataItem: T; colFields: string[] }[])
                                    .filter(obj => obj.dataItem[gridProps.dataItemKey] !== currentDataItem[gridProps.dataItemKey]);
                            }
                    ...
                    {
                        if (hSelectableProp?.selectMode === HSelectMode.Row) // === mode single row ===
                        {
                        ...
                        else // === mode multiple rows ===
                        {
                            const isSelected = selectedDataItemKeys.some(
                                key => currentDataItem[gridProps.dataItemKey] === key
                            );

                            const alreadySelected = (previousSelection as T[]).some(
                                item => item[gridProps.dataItemKey] === currentDataItem[gridProps.dataItemKey]
                            );
    ...
    switch (hSelectableProp?.selectMode)
    {
        case HSelectMode.Cell:
            selectionGridProps = {
                selectable: { enabled: true, cell: true, mode: 'single', drag: false },
                navigatable: true, select: hSelectableProp.select, onSelectionChange: (ev) => handleSelectionChange(ev, hSelectableProp?.onSelect)
            };
            break;
    ...
    let sortGridProps: Pick<GridProps, "sortable" | "defaultSort"| "sort" | "onSortChange">; //IHestiaGridSortProps;
    switch (hSortableProp?.mode)
    {
        case HSortableMode.Single:
            sortGridProps = {
                sortable: { mode: "single", allowUnsort: hSortableProp.allowUnsort ? hSortableProp.allowUnsort : false },
                onSortChange: hSortableProp.onSort,
                defaultSort: hSortableProp.defaultSort
            };
            break;
        case HSortableMode.Multiple:
            sortGridProps = {
                sortable: { mode: "multiple", allowUnsort: hSortableProp.allowUnsort ? hSortableProp.allowUnsort : false },
                onSortChange: hSortableProp.onSort,
                defaultSort: hSortableProp.defaultSort
            };
            break;
        default:
            sortGridProps = { sortable: undefined, sort: undefined, onSortChange: undefined, defaultSort: undefined };
    }
    ...
    const columns = useMemo(() => {
        return gridProps.children
            ? Array.isArray(gridProps.children)
                ? gridProps.children
                : [gridProps.children]
            : Array.isArray(hColumnsProps)
                ? hColumnsProps.map((c, i) => <GridColumn key={i} {...c} />) as unknown as ReactElement<GridColumnProps, typeof GridColumn>[]
                : [];
        ...
        else if(Array.isArray(props.hColumnsProps))
        {
            return props.hColumnsProps.map((c, i) => <GridColumn key={i} {...c} />) as unknown as ReactElement<GridColumnProps, typeof GridColumn>[];
        }
        else {
            return [];
        }*/
    }, [hColumnsProps, gridProps.children]);
    ...
    useEffect(() =>
    {
        const dataGridItems = gridProps.data.map(d => new DataGridItem(d/*, props.keySelector*/));
        setUnfilteredData(dataGridItems);
        setData(gridProps.data);
    }, [gridProps.data/*, props.keySelector*/]);
    ...
    const onDataStateChange = useCallback((ev: GridDataStateChangeEvent): void =>
    {
        ...
        console.log("onDataStateChange tri off")
    ...
    const finalData = useMemo(() =>
    {
        ...
        const processedData = process(data.slice(), dataState) as IHDataResult<DataGridItem<T>>;
    ...
    {
        const allGroups = groupBy(gridProps.data, dataState.group);
        setCollapsedGroup(
    ...
    {
        console.log("headerSelectionValue off")
            ...
            for (const fieldDeLaColonne in columnsVisibility)
            {
                ...
                if (columnsVisibility[fieldDeLaColonne] && item)
                {
                    ...
                    if (nestedValue)
                    {
                        ...
                        if (nestedValue instanceof Date && nestedValue.toLocaleDateString &&
    ...
    const gridChildren = useMemo(() =>
    {
        ...
        const gridToolBar = (isGroupable || isFilterable || isSortable) && (
            <GridToolbar className="grid-toolbar">
                {toolbarSettings.expandCollapseAllButton && isGroupable && (
                    <div className="expand-group-button">
        ...
        const children = columns.map((child, index) =>
        {
            if (child.type.displayName === 'KendoReactGridColumn') {
                if (child.props.columnType === 'checkbox') {
                    return <GridColumn key={index} {...child.props} headerSelectionValue={headerSelectionValue()} />;
                }
                else if (columnsState[child.props.title ?? child.props.field ?? ""]) {
                    return <GridColumn {...child.props} key={index} />;
                }
            ...
            if (/*col.props.field === newProps.selectedField*z/col.props.columnType === 'checkbox')
            {
        ...
        onExternalFilterChange, /*onGroupsToggle,*/ updateColumns, /*colPropsAPlat,*/ hasCollapsed, onGroupsCollapse, onGroupsExpand]); // TODO SD20250218 voir le deprecated de newProps.selectedField

    const gridRef = useRef(null);
    console.log("isGroupable",isGroupable);
    console.log("isSortable",isSortable);
    console.log("isSortable ? sortable : undefined",isSortable ? sortable : undefined);
    console.log("isFilterable",isFilterable);

    // "ref" est présent dans le code de la démo, je ne sais pas si cela sert réellement
    const renderedGridProps: GridProps & { ref: any; } = {
        ...newProps,
        filterable: isFilterable,//undefined,//On n'utilise pas les filtres de la même façon
    ...
    console.table({renderedGridProps})
    ...
    let className: string = "h-grid grid-helper-component grid-helper-component2";
    if (gridProps.className)
    {
        // On ajoute la ou les classes passées en props
        className = `${className} ${gridProps.className}`;
    }
    else
    {
        // Rien à faire
    ...
    if (isGroupable || isFilterable || isSortable)
    {
    ...
    console.log("gridChildren", gridChildren)
    console.table({renderedGridProps})
    // On crée une nouvelle grille qui sera celle affichée
    const GridComponent
        = (<Grid {...renderedGridProps} className={`h-grid ${className}`} loader={<span>Loading...</span>}>
            {gridChildren}
        </Grid>);
    ...
    return (
        <>
            {
                (isGroupable || isFilterable || isSortable) &&
                <>
                    {
                        toolbarSettings.pdfExportButton &&
