...
interface SelectionRestrictions<DataItem> {
    disabledColumns?: string[]; // liste des field des colonnes désactivées
    disabledRows?: Array<DataItem[keyof DataItem]>; // liste des clés désactivées
    disabledCells?: (item: DataItem, field: string) => boolean; // fonction personnalisée par cellule
}

interface GridHelperProps<DataItem> extends Omit<GridProps, 'onSelectionChange'> {
    toolbarSettings?: any;
    filterHighlights?: boolean;
    selectionRestrictions?: SelectionRestrictions<DataItem>;
...
export const GridHelper3 = (props) => {
    const {
    ...
    const onSelectionChange = React.useCallback(
    (event: GridSelectionChangeEvent) => {
        const { disabledColumns, disabledRows, disabledCells } = props.selectionRestrictions ?? {};
        const field = event.column?.field;
        const dataItemKey = props.dataItemKey!;
        const rowKey = event.dataItem[dataItemKey];

        // Priorité : colonne -> ligne -> cellule
        if (field && disabledColumns?.includes(field)) {
            event.preventDefault?.();
            event.stopPropagation?.();
            return; // bloque la sélection
        }

        if (disabledRows?.includes(rowKey)) {
            return;
        }

        if (field && disabledCells?.(event.dataItem, field)) {
            return;
        }
    ...
    [props.selectionRestrictions, props.onSelectionChange, GridProps]
);
